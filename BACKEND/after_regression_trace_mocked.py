...
MOCK_ANALYSES = {
    'HOMEBUYER': {'sub_queries': ['RERA Section 18 refund for delayed possession', 'Builder force majeure misuse'], 'domains': {'Property & Real Estate': 0.95}, 'explicit_sc_requested': True},
    'ADVERSE_POSSESSION': {'sub_queries': ['limitation period for adverse possession', 'permissive possession turning hostile'], 'domains': {'Property & Real Estate': 0.9}, 'explicit_sc_requested': True},
    'RAPE_CONSENT': {'sub_queries': ['statutory presumption of absence of consent', 'absence of physical resistance in rape'], 'domains': {'Criminal Law': 0.98}, 'explicit_sc_requested': True},
    'ILLEGAL_ARREST': {'sub_queries': ['detention beyond 24 hours without magistrate', 'grounds of arrest without warrant'], 'domains': {'Criminal Law': 0.96}, 'explicit_sc_requested': True},
    'NATURAL_JUSTICE': {'sub_queries': ['audi alteram partem natural justice', 'disciplinary dismissal fair hearing'], 'domains': {'Constitutional & Administrative': 0.95}, 'explicit_sc_requested': True}
}
...
def mocked_analyze(q, hist):
    for k, v in QUERIES.items():
        if v == q:
            return MOCK_ANALYSES[k]
    return {'sub_queries': [q], 'domains': {}, 'explicit_sc_requested': True}

rag_orchestrator._analyze_query_structured = mocked_analyze
# also mock the generate content inside _generate_answer to avoid api limits
def mocked_generate(*args, **kwargs):
    return 'Mocked answer'
rag_orchestrator._generate_answer = mocked_generate
...
with open('BACKEND/after_trace_mocked.json', 'w') as f:
    json.dump(results, f, indent=2)
print('Done. Saved to after_trace_mocked.json')
