...
import { classifyDomain, computeRetrievalMetrics, generateFivePartDossier } from '../data/caseEngine';
...
export const CivicNavigator: React.FC<CivicNavigatorProps> = ({
  ...
  const handleExecuteQuery = (textToQuery?: string) => {
    ...
    const resultDossier = generateFivePartDossier(text);

    // Realistic token streaming telemetry
    const logs = [
      ...
      'Cross-referencing Bharatiya Sakshya Adhiniyam 2023 evidentiary threshold...',
      'Computing Limitation Act 1963 countdowns and Single-Pass Legal Notice proforma...'
    ];

    let currentStep = 0;
    const interval = setInterval(() => {
      currentStep += 1;
      setStreamProgress(currentStep);
      setStreamingLog(logs[currentStep - 1] || 'Assembling court-ready dossier...');

      if (currentStep >= 5) {
        clearInterval(interval);
        setIsStreaming(false);
        setActiveDossier(resultDossier);
      }
    }, 190);
  };
